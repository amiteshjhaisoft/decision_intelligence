...
# --------------------------------- Paths / Constants ---------------------------------
# You can point these to your app's data folder if desired.
CONN_STORE = Path("connections.json")   # saved connector profiles live here
PIPE_STORE = Path("pipelines.json")     # saved pipelines live here
...
# If you have a registry elsewhere, import it. Otherwise, keep this minimal fallback.
REG_BY_ID: Dict[str, ConnectorMeta] = {
    # Example:
    # "azureblob": ConnectorMeta(id="azureblob", name="Azure Blob", icon="🟦"),
    # "weaviate": ConnectorMeta(id="weaviate", name="Weaviate", icon="🧠"),
}

# --------------------------------- Connections store helpers (read-only here) ---------------------------------
def _load_profiles_store() -> dict:
    """Return the full connections store as a dict; {} if missing/corrupt."""
    if not CONN_STORE.exists():
        return {}
    try:
        return json.loads(CONN_STORE.read_text(encoding="utf-8"))
...
def _profiles_for_connector(conn_id: str) -> List[str]:
    """Sorted list of profile names saved for a connector."""
...
def _pipelines_load_all() -> Dict[str, Dict[str, Any]]:
    """Load all pipelines; {} if missing/corrupt."""
...
def _pipeline_defaults() -> Dict[str, Any]:
    """Baseline shape for a pipeline record."""
    return {
        "name": "",
        "source_connector": "",
        "source_profile": "",
    ...
    if st.session_state["pipeline_form_open"]:
        ...
        with st.form("pipeline_form", clear_on_submit=False):
            ...
            with st.container(border=True):
                ...
                # Compute a safe default index for the selectbox
                dest_options = [""] + weaviate_profiles if weaviate_profiles else [""]
                try:
                    dest_index = dest_options.index(initial.get("destination_profile", ""))
                except ValueError:
                    dest_index = 0
