...
try:
    import keyring  # optional: OS keychain (Win/Mac/Linux)
except Exception:
    keyring = None
...
def resolve_secrets(obj: Any) -> Any:
    """
    Recursively resolve placeholders in dicts/lists:
      {"$env": "VAR"}                  -> os.environ["VAR"]
      {"$secret": "service/name"}      -> keyring.get_password(service, name)
      {"$secret": ["service","name"]}  -> same as above
    """
...
# Optional helper to set a secret once
